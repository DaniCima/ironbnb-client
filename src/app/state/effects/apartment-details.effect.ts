...
import {
  loadApartment,
  loadingApartment,
} from '../actions/apartment-details.action';
    ...
    return this.actions$.pipe(
      ...
      mergeMap((action) =>
        this.ironbnbAPIService
          .getApartmentById(action.id) // return the data from the API
          .pipe(
            // success case
            map(
              (apartment) => loadApartment({ apartment }) // dispatch the loadApartment action
            )
          )
      )
    );
